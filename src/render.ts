...
function getSubagentCallPreview(details: Pick<ChildRunDetails, "task" | "mode">, theme: any): string {
	const preview = details.task.length > 90 ? `${details.task.slice(0, 90)}...` : details.task;
	return `${theme.fg("toolTitle", theme.bold(TOOL_NAME))} ${theme.fg("accent", `[${details.mode}]`)}\n  ${theme.fg("dim", preview)}`;
}

export function renderExploreCall(args: { task?: string; mode?: unknown }, theme: any) {
	...
	const modeBadge = typeof args.mode === "string" ? ` ${theme.fg("accent", `[${args.mode}]`)}` : "";
	return new Text(`${theme.fg("toolTitle", theme.bold(TOOL_NAME))}${modeBadge}\n  ${theme.fg("dim", preview)}`, 0, 0);
}
...
export function renderSubagentResultBlock(
	...
	const failed = options.statusOverride ? options.statusOverride === "failed" : isSubagentFailure(details);
	...
	const status = options.statusOverride === "running"
	...
	const identity = options.showIdentity ? getSubagentCallPreview(details, theme) : undefined;

	if (!options.expanded) {
		const previewSource = finalOutput !== "(no output)"
			? finalOutput
			: toolCalls.at(-1)
		...
		const footer = usage ? `\n${theme.fg("dim", usage)}` : "";
		return new Text([identity, header, `${preview}${footer}`].filter(Boolean).join("\n"), 0, 0);
	}
	...
	container.addChild(new Text(theme.fg("dim", `${MODE_SPECS[details.mode].label} subagent · ${details.cwd}`), 0, 0));
	...
	container.addChild(new Markdown(finalOutput.trim() || "(no output)", 0, 0, getMarkdownTheme()));
	const stderr = options.errorText?.trim() || details.stderr.trim();
	if (stderr) {
		container.addChild(new Spacer(1));
		container.addChild(new Text(theme.fg(failed ? "error" : "dim", stderr), 0, 0));
	}
	if (usage) {
		container.addChild(new Spacer(1));
