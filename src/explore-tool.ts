...
export function registerExploreTool(pi: ExtensionAPI) {
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			if (!mode) throw new Error('explore_subagent requires mode: "shallow" or "deep"');
			const details = await runSubagent(mode, input.task, input.cwd ?? ctx.cwd, signal, onUpdate);
		...
		renderCall(args, theme) {
			return renderExploreCall(args as { task: string; mode?: ExploreMode }, theme);
		},
		renderResult(result, { expanded, isPartial }, theme) {
			...
			return renderSubagentResultBlock(details, finalOutput, { expanded, isPartial, showIdentity: false }, theme);
		},
	});
}
